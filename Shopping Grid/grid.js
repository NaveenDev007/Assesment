...
adiv.classList.add("card-container");

heading.innerHTML = "Clothing for Men and Women";
aheading.innerHTML = "Accessories for Men and Women";
...
for (p of productList) {
  ...
  image.setAttribute("src", p.preview);
  ...
  prod_name.innerHTML = p.name;
  prod_brand.innerHTML = p.brand;
  prod_price.innerHTML = `Rs ${p.price}`;

  if (!p.isAccessory) {
    div.appendChild(cards);
  } else {
    adiv.appendChild(cards);
  }
  ...
  details.appendChild(prod_brand);
  details.appendChild(prod_price);
}
